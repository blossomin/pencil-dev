...
def stat_labels(l):
    ml = np.max(l)
    return np.array([np.sum(l == i) for i in range(ml + 1)])
    ...
    for epoch in range(args.e):
        ...
        for step in tqdm(range(len(train_dataloader))):
            ...
            if SPLIT:
                ...
                print("Training from split {0}, with labels histogram {1}".format(step % SPLIT_JOIN_COUNT, stat_labels(labels.numpy())))
            else:
