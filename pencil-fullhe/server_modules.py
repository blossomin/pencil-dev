...
class Linear(ServerModule):
  ...
  def collaborate_matmul(self, helper, x_server, weight, output_size):
    ...
    y_client_cipher1 = self.crypto.matmul_pack_outputs(helper, y_client_cipher)
    s = self.crypto.field_random_mask(output_size)
    s_encoded = self.crypto.matmul_encode_y(helper, s)
    self.crypto.add_plain_inplace(y_client_cipher1, s_encoded)
    self.send(self.crypto.matmul_serialize_y(helper, y_client_cipher1))
  ...
  def backward_calculate_partial_A(self, partial_y_server):
    ...
    # print(f"partial_y_server_encoded:size:{partial_y_server_encoded.size()}, col:{partial_y_server_encoded.columns()}, row:{partial_y_server_encoded.rows()}")
    # print(f"x_client_ciper: size:{x_client_cipher.size()}, col:{x_client_cipher.columns()}, row:{x_client_cipher.rows()}")
    w = self.crypto.matmul(helper, partial_y_client_cipher, x_server_encoded)
    # w = self.crypto.matmul_pack_outputs(helper, w)
    w1 = self.crypto.matmul_pack_outputs(helper, w)
    w2 = self.crypto.matmul_reverse(helper, partial_y_server_encoded, x_client_cipher)
    # w2 = self.crypto.matmul_pack_outputs(helper, w2)
    self.crypto.add_inplace(w, w2)
    s = self.crypto.field_random_mask(self.output_dims * self.input_dims)
    s_encoded = self.crypto.matmul_encode_y(helper, s)
    # print(f"w: size:{w.size()}, col:{w.columns()}, row:{w.rows()}")
    # print(f"w1: size:{w1.size()}, col:{w1.columns()}, row:{w1.rows()}")
    # print(f"s: {s}")
    # print(f"s_encoded: size:{s_encoded.size()}, col:{s_encoded.columns()}, row:{s_encoded.rows()}")
    self.crypto.add_plain_inplace(w1, s_encoded)
    self.send(self.crypto.matmul_serialize_y(helper, w1))
...
class Conv2d(ServerModule):
  ...
  def backward_calculate_partial_A(self, partial_y_server, partial_y_server_encoded=None, partial_y_client_cipher=None):
    ...
    w = self.crypto.conv2d_reverse(helper, x_server_encoded, partial_y_client_cipher)
